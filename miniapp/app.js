...
function render(){
  ...
  for(let pot=1;pot<=4;pot++){
    ...
    teams.filter(t=>t.pot===pot && t.country!=='TR').forEach(team=>{
      const selected=picks[active].has(team.id); const allowed=selected||canSelect(active,team).ok;
      const b=document.createElement('button'); b.type='button'; b.className=`team-card${selected?' selected':''}${!allowed?' disabled':''}`;
      b.innerHTML=`<span class="check">${selected?'✓':''}</span><span class="team-logo" role="img" aria-label="${team.name}" style="--x:${team.spriteX};--y:${team.spriteY}"></span><span class="team-name">${team.name}</span>`;
      b.addEventListener('click',()=>toggle(team)); grid.appendChild(b);
    });
...
document.querySelectorAll('.team-tab').forEach(btn=>btn.addEventListener('click',()=>{
  active=btn.dataset.target; document.querySelectorAll('.team-tab').forEach(x=>x.classList.toggle('active',x===btn)); setMessage(''); render(); window.scrollTo({top:document.querySelector('.team-tabs').offsetTop-4,behavior:'smooth'});
}));
...
submitBtn.addEventListener('click',()=>{
  ...
  const payload={type:'sl_draw_submission',pusulabet_username:username,fenerbahce:[...picks.fenerbahce],galatasaray:[...picks.galatasaray]};
  submitBtn.disabled=true; submitBtn.textContent='Kaydediliyor…';
  try{ tg.sendData(JSON.stringify(payload)); tg.HapticFeedback?.notificationOccurred('success'); setMessage('Tahminin bota gönderildi.'); setTimeout(()=>tg.close(),700); }
  catch(e){ submitBtn.disabled=false; submitBtn.textContent='Tahminimi Kaydet'; setMessage('Gönderim başarısız. Mini App’i bottan tekrar aç.'); }
});
